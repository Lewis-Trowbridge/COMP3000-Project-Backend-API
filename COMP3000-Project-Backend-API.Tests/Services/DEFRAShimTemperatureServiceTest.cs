...
namespace COMP3000_Project_Backend_API.Tests.Services
{
    public class DEFRAShimTemperatureServiceTest
    {
        ...
        [Fact]
        public async void DEFRAShimTemperatureService_Get_MakesCorrectHttpRequestWithTimestamp()
        {
        ...
        [Fact]
        public async void DEFRAShimTemperatureService_Get_MakesCorrectHttpRequestWithNoTimestamp()
        {
            var testAddress = DEFRAShimTemperatureService.BaseAddress + $"/data?site=test";
            var handler = new Mock<HttpMessageHandler>();
            handler.SetupRequest(HttpMethod.Get, testAddress).ReturnsResponse(System.Net.HttpStatusCode.OK, ValidResponseJSON);
            ...
            await service.GetTemperatureInfo(TestMetadata, null);
        ...
        [Fact]
        public async void DEFRAShimTemperatureService_Get_ReturnsDataFromResponseWithoutTimestamp()
        {
            var testAddress = DEFRAShimTemperatureService.BaseAddress + $"/data?site=test";
            var handler = new Mock<HttpMessageHandler>();
            handler.SetupRequest(HttpMethod.Get, testAddress).ReturnsResponse(System.Net.HttpStatusCode.OK, ValidResponseJSON);
            ...
            var actual = await service.GetTemperatureInfo(TestMetadata, null);
